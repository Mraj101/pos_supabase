...
function StatCard({ icon: Icon, label, value, sub, color = 'rose', link }) {
  ...
  const card = (
    <div className="card flex items-start gap-3 hover:shadow-md transition-shadow">
      <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${colorMap[color]}`}>
        <Icon size={20} />
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-xs text-charcoal-soft uppercase tracking-wide mb-0.5">{label}</p>
        <p className="font-display text-xl sm:text-2xl font-semibold text-charcoal">{value}</p>
        {sub && <p className="text-xs text-charcoal-soft mt-0.5">{sub}</p>}
      </div>
      {link && <ChevronRight size={16} className="text-gray-300 flex-shrink-0 mt-1" />}
    </div>
...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setStats({
        todayTotal:    (todaySalesRaw || []).reduce((s, r) => s + Number(r.total_amount), 0),
        totalDue:      (balances || []).reduce((s, r) => s + Number(r.balance_due), 0),
        lowStockCount: lowStockRaw?.length || 0,
        dueCustCount:  balances?.length || 0,
      })
      setRecentSales(sales || [])
  ...
  return (
    <div className="space-y-5 lg:space-y-8">
      ...
      <div className="flex items-end justify-between">
        <div>
          <h2 className="font-display text-3xl lg:text-4xl font-semibold text-charcoal">Dashboard</h2>
          <p className="text-charcoal-soft text-sm mt-1 hidden sm:block">
            {new Date().toLocaleDateString('en-BD', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
          </p>
        </div>
        <Link to="/new-sale" className="btn-primary flex items-center gap-2 text-sm">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
        <StatCard icon={TrendingUp}    label="Today's Sales"  value={fmt(stats.todayTotal)}  color="rose" />
        <StatCard icon={Users}         label="Total Due"      value={fmt(stats.totalDue)}    sub={`${stats.dueCustCount} customers`} color="gold" link="/customers" />
        <StatCard icon={AlertTriangle} label="Low Stock"      value={stats.lowStockCount}    sub="variants ≤ 5" color="orange" />
        <StatCard icon={Users}         label="Due Customers"  value={stats.dueCustCount}     color="green" link="/customers" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 card p-0 overflow-hidden">
          <div className="px-4 sm:px-6 py-4 border-b border-cream-dark flex justify-between items-center">
            <h3 className="font-display text-lg sm:text-xl font-semibold">Recent Sales</h3>
            <Link to="/reports" className="text-rose text-xs hover:underline">View all</Link>
          </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                {recentSales.map(sale => {
                  const due = Number(sale.total_amount) - Number(sale.amount_paid)
                  return (
                    <tr key={sale.id} className="hover:bg-cream/50">
                      <td className="table-cell font-medium">{sale.customers?.name || '—'}</td>
                      <td className="table-cell">{fmt(sale.total_amount)}</td>
                      <td className="table-cell text-green-600">{fmt(sale.amount_paid)}</td>
                      <td className="table-cell">
                        {due > 0 ? <span className="text-red-500 font-medium">{fmt(due)}</span> : <span className="text-green-500">Paid</span>}
                      </td>
                      <td className="table-cell text-charcoal-soft text-xs">{new Date(sale.sale_date).toLocaleDateString('en-BD')}</td>
                    </tr>
                  )
                })}
                {recentSales.length === 0 && (
          ...
          <div className="sm:hidden divide-y divide-cream-dark">
            {recentSales.map(sale => {
              ...
              return (
                <div key={sale.id} className="px-4 py-3 flex items-center justify-between">
                  <div>
                    <p className="font-medium text-sm text-charcoal">{sale.customers?.name || '—'}</p>
                    <p className="text-xs text-charcoal-soft">{new Date(sale.sale_date).toLocaleDateString('en-BD')}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-semibold">{fmt(sale.total_amount)}</p>
                    {due > 0
            ...
            {recentSales.length === 0 && <p className="text-center py-8 text-sm text-charcoal-soft">No sales yet</p>}
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="card p-0 overflow-hidden">
            ...
            <div className="divide-y divide-cream-dark">
              ...
              {topDue.length === 0 && <p className="px-5 py-4 text-sm text-charcoal-soft text-center">No dues 🎉</p>}
            </div>
          </div>
          ...
          <div className="card p-0 overflow-hidden">
            ...
            <div className="divide-y divide-cream-dark">
              ...
              {lowStock.length === 0 && <p className="px-5 py-4 text-sm text-charcoal-soft text-center">Stock healthy ✓</p>}
            </div>
          </div>
        </div>
      </div>
