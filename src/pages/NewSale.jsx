...
export default function NewSale() {
  ...
  const [error,           setError]           = useState('')
  const [success,         setSuccess]         = useState(false)
  // Mobile: toggle between cart view and summary view
  const [mobileTab, setMobileTab] = useState('items') // 'items' | 'summary'

  useEffect(() => {
    if (!customerQuery.trim()) { setCustomerResults([]); return }
    const t = setTimeout(async () => {
      const { data } = await supabase.from('customers').select('id,name,phone')
        .or(`name.ilike.%${customerQuery}%,phone.ilike.%${customerQuery}%`).limit(6)
      setCustomerResults(data || [])
    }, 300)
    return () => clearTimeout(t)
  }, [customerQuery])
  ...
  const selectCustomer = (c) => { setCustomer(c); setCustomerQuery(c.name); setCustomerResults([]) }
  ...
  const addToCart = (variant) => {
    ...
    setVariantQuery(''); setVariantResults([]); setShowVariants(false)
  }
  ...
  const updatePrice = (id, price) => setCart(cart => cart.map(i => i.id === id ? { ...i, unit_price: Number(price) || 0 } : i))
  const removeItem  = (id)        => setCart(cart => cart.filter(i => i.id !== id))
  ...
  const resetForm = () => {
    setCustomer(null); setCustomerQuery(''); setCustomerBalance(null)
    setCart([]); setAmountPaid(''); setNote(''); setError(''); setSuccess(false); setMobileTab('items')
  }

  if (success) return (
    <div className="flex flex-col items-center justify-center min-h-96 text-center space-y-4 px-4">
      <div className="w-20 h-20 bg-green-50 rounded-full flex items-center justify-center">
        <CheckCircle size={40} className="text-green-500" />
      </div>
      <h2 className="font-display text-3xl font-semibold text-charcoal">Sale Recorded!</h2>
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      <h2 className="font-display text-3xl lg:text-4xl font-semibold text-charcoal">New Sale</h2>

      {/* Mobile tab switcher */}
      <div className="flex lg:hidden gap-1 bg-cream-dark p-1 rounded-xl">
        <button onClick={() => setMobileTab('items')}
          className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${mobileTab === 'items' ? 'bg-white text-charcoal shadow-sm' : 'text-charcoal-soft'}`}>
          Items {cart.length > 0 && `(${cart.reduce((s,i)=>s+i.qty,0)})`}
        </button>
        <button onClick={() => setMobileTab('summary')}
          className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${mobileTab === 'summary' ? 'bg-white text-charcoal shadow-sm' : 'text-charcoal-soft'}`}>
          Summary {total > 0 && `· ${fmt(total)}`}
        </button>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6">
        {/* Left: Customer + Items — hidden on mobile when summary tab */}
        <div className={`lg:col-span-2 space-y-4 ${mobileTab === 'summary' ? 'hidden lg:block' : 'block'}`}>

          {/* Customer */}
          <div className="card">
            <h3 className="font-display text-lg sm:text-xl font-semibold mb-3">Customer</h3>
            <div className="relative">
              <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
              <input className="input pl-9" placeholder="Search by name or phone…" value={customerQuery}
                onChange={e => { setCustomerQuery(e.target.value); setCustomer(null) }} />
              {customerResults.length > 0 && (
                <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-white border border-cream-dark rounded-xl shadow-lg overflow-hidden">
                  {customerResults.map(c => (
          ...
          {/* Product Search */}
          <div className="card">
            <h3 className="font-display text-lg sm:text-xl font-semibold mb-3">Add Items</h3>
            <div className="relative">
              <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
              <input className="input pl-9" placeholder="Search product or SKU…" value={variantQuery}
                onChange={e => setVariantQuery(e.target.value)}
                onFocus={() => variantResults.length > 0 && setShowVariants(true)} />
              {showVariants && variantResults.length > 0 && (
                <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-white border border-cream-dark rounded-xl shadow-lg overflow-hidden max-h-64 overflow-y-auto">
                  {variantResults.map(v => (
                    <button key={v.id} onClick={() => addToCart(v)}
                      ...
                      <div>
                        <p className="text-sm font-medium">{v.product_name}</p>
                        <p className="text-xs text-charcoal-soft">{[v.sku, v.size, v.color].filter(Boolean).join(' · ')}</p>
                      </div>
            ...
            {cart.length > 0 ? (
              <div className="mt-4 space-y-2">
                {cart.map(item => (
                  <div key={item.id} className="bg-cream/60 rounded-xl px-3 py-3">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2 flex-1 justify-end">
                        <div className="relative w-28">
                          <span className="absolute left-2 top-1/2 -translate-y-1/2 text-charcoal-soft text-xs">৳</span>
                          <input type="number" className="input w-full pl-5 text-right text-sm py-1.5" value={item.unit_price}
                            onChange={e => updatePrice(item.id, e.target.value)} />
                        </div>
                        <p className="text-sm font-semibold text-charcoal w-20 text-right">{fmt(item.qty * item.unit_price)}</p>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {/* Right: Summary — hidden on mobile when items tab */}
        <div className={`${mobileTab === 'items' ? 'hidden lg:block' : 'block'}`}>
          <div className="card lg:sticky lg:top-8">
            ...
            <div className="mt-5 space-y-3">
              <div>
                <label className="label">Amount Paid ৳</label>
                <input type="number" className="input text-right font-semibold" placeholder="0"
                  value={amountPaid} onChange={e => setAmountPaid(e.target.value)} max={total} />
                <div className="flex gap-3 mt-2">
                  <button onClick={() => setAmountPaid(String(total))} className="text-xs text-rose hover:underline">Full payment</button>
                  <span className="text-xs text-gray-300">|</span>
                  <button onClick={() => setAmountPaid('0')} className="text-xs text-rose hover:underline">Credit only</button>
                </div>
              </div>
              {due !== 0 && total > 0 && (
                <div className={`flex justify-between text-sm font-semibold p-3 rounded-lg ${due > 0 ? 'bg-red-50 text-red-500' : 'bg-green-50 text-green-600'}`}>
              ...
              {error && <p className="text-red-500 text-xs">{error}</p>}
              <button onClick={handleSubmit} disabled={saving || cart.length === 0 || !customer}
                className="btn-primary w-full py-3 text-base flex items-center justify-center gap-2">
                <ShoppingCart size={17} />
                {saving ? 'Processing…' : 'Confirm Sale'}
              </button>
