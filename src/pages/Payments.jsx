...
export default function Payments() {
  ...
  const loadRecent = () =>
    supabase.from('payments').select('id,amount,payment_date,note,customers(name)')
      .order('payment_date', { ascending: false }).limit(10)
      .then(({ data }) => setRecent(data || []))

  useEffect(() => { loadRecent() }, [success])

  useEffect(() => {
    if (!query.trim()) { setResults([]); return }
    const t = setTimeout(async () => {
      const { data } = await supabase.from('customers').select('id,name,phone')
        .or(`name.ilike.%${query}%,phone.ilike.%${query}%`).limit(6)
      setResults(data || [])
    }, 300)
    return () => clearTimeout(t)
  }, [query])
  ...
  const selectCustomer = async (c) => {
    setCustomer(c); setQuery(c.name); setResults([]); setError(''); setAmount(''); setSaleId('')
    const [{ data: bal }, { data: openSales }] = await Promise.all([
      supabase.from('customer_balances').select('balance_due').eq('customer_id', c.id).single(),
      supabase.from('sales').select('id,total_amount,amount_paid,sale_date')
        .eq('customer_id', c.id).order('sale_date', { ascending: false }).limit(20),
    ])
    setBalance(bal?.balance_due || 0)
    setSales((openSales || []).filter(s => Number(s.total_amount) - Number(s.amount_paid) > 0))
  }
  ...
  const handleSubmit = async () => {
    if (!customer)                          { setError('Select a customer'); return }
    if (!amount || Number(amount) <= 0)     { setError('Enter a valid amount'); return }
    if (Number(amount) > Number(balance))   { setError(`Exceeds balance (${fmt(balance)})`); return }
    setSaving(true); setError('')
    const { error: err } = await supabase.rpc('record_payment', {
      p_customer_id: customer.id, p_amount: Number(amount),
      p_sale_id: saleId || null, p_note: note || null,
    })
    setSaving(false)
    if (err) { setError(err.message); return }
    setSuccess(true)
  ...
  return (
    <div className="space-y-5 lg:space-y-6">
      <h2 className="font-display text-3xl lg:text-4xl font-semibold text-charcoal">Record Payment</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="card">
            ...
            <div className="relative">
              <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
              <input className="input pl-9" placeholder="Search by name or phone…" value={query}
                onChange={e => { setQuery(e.target.value); setCustomer(null); setBalance(null) }} />
              {results.length > 0 && (
                <div className="absolute z-20 top-full left-0 right-0 mt-1 bg-white border border-cream-dark rounded-xl shadow-lg overflow-hidden">
                  {results.map(c => (
            ...
            {customer && balance !== null && (
              <div className="mt-3 bg-rose-light/30 rounded-xl p-4 flex items-center justify-between">
                <div>
                  <p className="font-semibold text-charcoal">{customer.name}</p>
                  <p className="text-xs text-charcoal-soft">{customer.phone}</p>
                </div>
          ...
          {customer && (
            <div className="card space-y-4">
              <h3 className="font-display text-lg sm:text-xl font-semibold">Payment Details</h3>
              {sales.length > 0 && (
                <div>
                  <label className="label">Link to Sale (optional)</label>
                  {/* Custom list replaces <select> — native options overflow on mobile */}
                  <div className="border border-gray-200 rounded-lg overflow-hidden text-sm">
                    <button
                      type="button"
                      onClick={() => setSaleId('')}
                      className={`w-full flex items-center justify-between px-3 py-2.5 text-left border-b border-gray-100 transition-colors ${saleId === '' ? 'bg-rose-light/60 text-rose font-medium' : 'hover:bg-cream/70 text-charcoal-soft'}`}
                    >
                      <span>General payment</span>
                      {saleId === '' && <span className="text-xs">✓</span>}
                    </button>
                    {sales.map(s => {
                      const due = Number(s.total_amount) - Number(s.amount_paid)
                      return (
                        <button
                          ...
                          className={`w-full flex items-center justify-between px-3 py-2.5 text-left border-b border-gray-100 last:border-0 transition-colors ${saleId === s.id ? 'bg-rose-light/60 text-rose font-medium' : 'hover:bg-cream/70 text-charcoal'}`}
                        >
                          <span className="text-xs text-charcoal-soft">{new Date(s.sale_date).toLocaleDateString('en-BD')}</span>
                          <span className={`font-semibold text-xs ml-2 flex-shrink-0 ${saleId === s.id ? 'text-rose' : 'text-red-500'}`}>Due: {fmt(due)}</span>
                        </button>
                      )
                    })}
                  </div>
              ...
              <div>
                <label className="label">Payment Amount ৳ *</label>
                <input type="number" className="input text-right text-xl font-semibold" placeholder="0"
              ...
              <div>
                <label className="label">Note (optional)</label>
                <input className="input" placeholder="bKash, cash, etc." value={note} onChange={e => setNote(e.target.value)} />
              </div>
              {error && <p className="text-red-500 text-sm">{error}</p>}
              {success ? (
                <div className="flex items-center justify-center gap-2 py-3 text-green-600 font-medium">
                ...
                <button onClick={handleSubmit} disabled={saving || !customer || !amount}
                  className="btn-primary w-full py-3 flex items-center justify-center gap-2">
                  <CreditCard size={17} />
                  {saving ? 'Recording…' : 'Record Payment'}
                </button>
              ...
              <div key={p.id} className="px-5 py-3">
                <div className="flex items-center justify-between">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-charcoal truncate">{p.customers?.name}</p>
                    <p className="text-xs text-charcoal-soft">{p.note || '—'}</p>
                  </div>
